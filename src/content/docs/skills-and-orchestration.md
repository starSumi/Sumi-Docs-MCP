...
This repository does not currently ship a runtime Skill and therefore does not
create an `.agent/` or `.agents/skills/` directory. A client-specific Skill
belongs in the owning client or user catalog, where its activation lifecycle can
be tested.

Open an issue before adding a reusable integration. Include trigger and

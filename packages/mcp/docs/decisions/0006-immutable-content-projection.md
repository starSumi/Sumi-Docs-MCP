...
Publication follows one logical writer state machine:

```text
AUTHORING -> VALIDATING -> STAGING -> HASHING -> SEALED
          -> PUBLISH_SNAPSHOT -> COMPARE_AND_SET_CURRENT -> PROJECT_V1
```

Any validation failure or source change during the build aborts publication
before `current.json` or v1 changes. Existing immutable snapshots may be kept
for rollback and in-flight consumers.
...
and two independent cursor consumers. Rollback removes the v2 locator while the
unchanged v1 projection remains available.

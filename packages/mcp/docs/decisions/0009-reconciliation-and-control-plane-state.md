...
Codex demonstrates a useful separation: ordered append-only evidence, an
indexed SQLite projection, explicit lifecycle acknowledgements, and parent-child
edges. Sumi Docs needs the separation, not the Codex conversation schema.
...
- idempotent replay and compare-and-set promotion of `current.json`.
...
This adapts the scoped lifecycle and disposer pattern demonstrated by Cordis,
but does not add Cordis as a dependency. A dynamic plugin container is not
justified for the four-tool MCP server.

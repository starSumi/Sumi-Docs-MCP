...
Run `doctor --json` first. Paths are relative or redacted by default. Add
`--show-paths` only during local interactive diagnosis and do not publish that
output. Client errors and stderr diagnostics never include stack traces or URL
credentials.

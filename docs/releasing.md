...
SHA from the latest private `main`, runs the release suite without OIDC or
attestation authority, and uploads static archives, SHA-256 checksums, and raw
performance evidence. It does not deploy the site.

Provenance attestation is a separate protected job. It runs only when the
repository variable `ENABLE_PRIVATE_ATTESTATION` is `true` and the
`candidate-attestation` environment is available and protected. Keep the
variable unset when the private repository plan cannot enforce that boundary;

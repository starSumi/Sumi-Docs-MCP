...
本文是 Sumi Docs 的产品需求文档（PRD）。不再使用含义不明确的 PDR 建立第二份
需求真相。设计决策进入 ADR，交付证据进入 checkpoint 和 CI。
...
| 发布负责人 | 审核不可变证据，在任何公开操作之前明确接受或拒绝私有候选。             |
...
Node.js SEA 的目标是首次 `tools/list` 少于 50 ms，100 ms 为阻断上限。在当前 30 次
测量证明通过之前，该门保持失败。签名和公开发布隐私同样是阻断项，除非负责人明确
记录例外。
